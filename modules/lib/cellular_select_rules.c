...
error select_rules( char* rules, struct rules* zasady ) {
	...
	if( (file_rules = fopen( rules, "r" )) == NULL ) {
		sprintf( error_message, "Nie udało się otworzyć pliku z zasadami generacji: %s.", rules );
		return FOPEN_ERROR;
	}

	while( 1 ) {	
		if( fscanf( file_rules, "%d %d", &cell_status, &elements ) != 2 ) {
			sprintf( error_message, "Niepoprawny format danych wejściowych pliku z zasadami (1): %s.", rules );
			return FORMAT_ERROR;
		}

		if( (cell_status != 1 && cell_status != 0) || elements < 0 ) {
			sprintf( error_message, "Niepoprawny format danych wejściowych pliku z zasadami (2): %s.", rules );
			return FORMAT_ERROR;
		}
		...
		for( i=0; i<elements; i++ ) {
			if( ( fscanf( file_rules, "%d", &temp[i] ) != 1) || temp[i] > 8 || temp[i] < 0 ) {
				sprintf( error_message, "Niepoprawny format danych wejściowych pliku z zasadami (3): %s.", rules );
				return FORMAT_ERROR;
			}  
		}
